...
namespace ApiCatalogGames.Repositories
{
    public class GameSqlServerRepository : IGameRepository
    {
        ...
        public async Task<List<Game>> Get(int page, int quantity)
        {
            ...
            var comando = $"select * from Games order by id offset {((page - 1) * quantity)} rows fetch next {quantity} rows only";
        ...
        public async Task<List<Game>> Get(string name, string producer)
        {
            ...
            var comando = $"select * from Games where Name = '{name}' and Producer = '{producer}'";
        ...
        {
            var comando = $"insert Games (Id, Name, Producer, Price) values ('{game.Id}', '{game.Name}', '{game.Producer}', {game.Price.ToString().Replace(",", ".")})";
        ...
        {
            var comando = $"update Games set Name = '{Game.Name}', Producer = '{Game.Producer}', Price = {Game.Price.ToString().Replace(",", ".")} where Id = '{Game.Id}'";
